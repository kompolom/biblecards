...
export const versesSlice = createSlice({
  ...
  reducers: {
    versesLoaded: (state, action: PayloadAction<Excerpt[]>) => {
      versesAdapter.addMany(
        state.entities,
        action.payload.map((excerpt) => ({
          id: excerpt.id,
          text: excerpt.text,
        })),
      );
    ...
    verseAdded: (state, action: PayloadAction<IVerse>) => {
      versesAdapter.addOne(state.entities, {
    ...
    verseDeleted: (state, action: PayloadAction<string>) => {
...
export const versesListSelector = (rootState: VersesStateShape) =>
  selectAll(rootState.verses.entities);
export const getVerseById = createSelector(
  [
    (rootState: VersesStateShape) => rootState.verses.entities,
